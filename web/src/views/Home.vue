<template>
  <div class="flex flex-1 flex-col">
    <main class="mx-auto flex w-full max-w-3xl flex-1 items-center p-8">
      <div class="w-full space-y-6 text-center">
        <div class="space-y-3">
          <div
            class="text-muted-foreground text-sm font-semibold tracking-[0.2em] uppercase"
          >
            ScotDance.app
          </div>
          <h2 class="font-serif text-7xl font-medium tracking-tight">
            Highland dance, tracked.
          </h2>
          <p class="text-muted-foreground font-serif text-2xl">
            Browse competitions, track dancers, and view results in real-time.
          </p>
        </div>
      </div>
    </main>
  </div>
</template>
