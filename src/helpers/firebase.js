...
if (isDev()) {
  ...
  // use the page's hostname so emulators are reachable when accessing the dev
  // server from another device on the LAN (e.g. Safari at 192.168.x.x:3000)
  const emulatorHost = window.location.hostname;
  firebase.auth().useEmulator(`http://${emulatorHost}:${9099}/`, { disableWarnings: true });
  firebase.database().useEmulator(emulatorHost, 9009);
